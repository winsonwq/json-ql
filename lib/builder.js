...
export default class Builder {
  ...
  build(query, limit, offset) {
    ...
    const sql = this.concatSnippets(
      ...
      this.buildFrom(context, limit, offset, this.getAllJoinPaths(context.parsedExp.paths, context.parsedFilters)),
  ...
  parseObj = R.curry((query, row) => {
    ...
    const availablePropPaths = R.filter(p => p.type == AGGRAGATION_PROP || p.type == FIELD_PROP, paths);
  ...
  mergeParsedObjs(parsedObjs) {
    ...
    return parsedObjs
      ...
      .reduce(function(sofar, curr) {
        ...
        if (target) {
          const mergedArrayProps = R.keys(getArrays(target.value))
            .reduce((sofar, key) => {
              return R.merge(sofar, {
                [key]: _this.mergeParsedObjs(target.value[key].concat(curr.value[key]))
              }) ;
            }, {});

          return [...R.reject(R.equals(target), sofar), { key: curr.key, value: R.merge(curr.value, mergedArrayProps) }];
        } else {
          return [...sofar, curr];
        }
      }, [])
  ...
  buildFrom(context, limit, offset = 0, joinPaths) {
    if (joinPaths.length == 0) {
      const schemaNames = R.keys(context.mapping);
      let { tableName, alias } = context.mapping[schemaNames[0]];

      return FROM(SELECT_FROM_WITH_PAGER(tableName, limit, offset), alias);
    } else {
      return this.buildFromAndJoins(context, limit, offset, joinPaths);
    }
  }

  buildFromAndJoins(context, limit, offset = 0, joinPaths) {
    const ret = joinPaths
    ...
    return this.concatSnippets(
      FROM(SELECT_FROM_WITH_PAGER(rootJoin.from.tableName, limit, offset), rootJoin.from.alias),
...
const alias = tableName => `${tableName}_${rndm(2)}`;
